...
def load_inv_lut(path: Path = INV_LUT_HEX) -> tuple[np.ndarray, str]:
    """Load the RTL LUT, or recreate its 10-bit-index/Q2.14 table.

    The RTL index is round(sum_sq * 620 / 2^16).  Therefore an index i
    represents RMS^2 = i * 2^16 / (620 * EMBED_DIM).
    """
    if path.exists():
        return load_hex_u16(path), str(path)
    idx = np.arange(1024, dtype=np.float64)
    values = np.empty(1024, dtype=np.int64)
    values[0] = 0xFFFF
    values[1:] = np.rint(
        (1 << 14) * np.sqrt((620.0 * EMBED_DIM) / ((1 << 16) * idx[1:]))
    ).astype(np.int64)
    values = np.clip(values, 0, 0xFFFF)
    return values, "generated:index=round(sum_sq*620/2^16),value=round((1/rms)*2^14)"
...
def export_case(args: argparse.Namespace, out_dir: Path) -> dict[str, Any]:
    ...
    inv_lut, inv_lut_source = load_inv_lut()
