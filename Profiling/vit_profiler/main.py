...
from .roofline import (
    plot_roofline,
    plot_bar_compare,
    plot_memory_access_compare,
    plot_memory_usage_compare,
    plot_norm_softmax_macs_compare,
)
from .optimization_analysis import export_optimization_analysis


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="ViT profiling package: Baseline-B FP32 all-DRAM tiled vs optimized INT8 RTL-aware 8x8 dataflow.")
    ...
    p.add_argument("--output-dir", "-o", default="outputs_rtl8", help="output directory")
    p.add_argument("--clock-mhz", type=float, default=25.0, help="PL clock in MHz")
    ...
    p.add_argument("--bram-service-bpc", type=float, default=8.0, help="local BRAM service bandwidth in bytes/cycle")
    p.add_argument("--leakage-w", type=float, default=0.20, help="leakage/static power assumption in W")
    p.add_argument("--gelu-page-words", type=int, default=1024, help="GELU page BRAM size in INT8 words")
    p.add_argument("--optimistic-gelu-page", action="store_true", help="assume FC2 hidden pages are loaded from DDR once instead of once per FC2 output tile")
...
def main() -> None:
    ...
    hw = HardwareConfig(
        ...
        gelu_page_words=args.gelu_page_words,
        gelu_ddr_reload_per_fc2_output_tile=not args.optimistic_gelu_page,
    )
    ...
    if not args.no_plots:
        ...
        if not group_df.empty:
            plot_roofline(group_df, out / "roofline_groups.png", title="Group-level DRAM roofline", show_label_arrows=False)
        plot_bar_compare(df, "macs", out / "macs_by_section.png", title="True MACs by section", ylabel="MACs")
        ...
        plot_norm_softmax_macs_compare(df, out / "cycles_norm_softmax.png", metric="cycles_total", title="Norm and Softmax cycles by section", ylabel="cycles")
