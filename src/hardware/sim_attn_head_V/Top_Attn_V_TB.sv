...
module Top_Attn_V_TB;
    ...
    logic [TOKEN_TILE*CHANNEL_TILE*DATA_W-1:0] residual_tile_i; // 2048-bit 殘差輸入 X
    ...
    // RMSNorm 統計交握介面 (修復 Vivado 2019.2 埠口類型錯誤：單個 32-bit Packed 連線)
    logic        stat_valid_o;
    logic        stat_ready_i;
    ...
    // (A) 實例化 16x16 脈動陣列核心 (此處 Act = Softmax score S, Weight = Value V)
    Systolic u_Systolic (
        .clk(clk),
        .rst(rst),
        .en(en),
    ...
    PPU #(
        ...
        .residual_tile_i(residual_tile_i), // 餵入原始殘差 X 矩陣
    ...
    logic [31:0] residual_bram_mem [0:63];   // 存放殘差 X 矩陣
    logic [7:0]  golden_attn_v_mem [0:255];  // 預期輸出特徵圖 X_mid = X + O
    ...
    //    安全設計：避免與 initial 區塊產生 Race Condition 導致 Token 15 採樣為 0
    // =================================================================
    always_ff @(posedge clk) begin
        if (rst) begin
    ...
    // =================================================================
    initial begin
        // 強制檢查模擬目錄下是否有 hex 檔案，避免因檔案讀取失敗噴出未定義 'x'
        $dumpfile("top_attn_v_pipeline.vcd");
        $dumpvars(0, Top_Attn_V_TB);

        // 加載由 Python 生成的專用測試資料
        $readmemh("tb_attn_v_act.hex",      act_bram_mem);
        $readmemh("tb_attn_v_weight.hex",   w_bram_mem);
        $readmemh("tb_attn_v_bias.hex",     bias_bram_mem);
        $readmemh("tb_attn_v_residual.hex", residual_bram_mem);
        $readmemh("tb_attn_v_golden.hex",   golden_attn_v_mem);
        $readmemh("tb_attn_v_sum_sq.hex",   golden_sum_sq);
        $display("[System-TB] All Attn_V (S x V) verification hex files loaded.");
        ...
        // 配置 PPU 暫存器
        ppu_mode_i = 2'b00;          // 2'b00 代表 Attention Output Phase (X_mid = X + O)
        scaling_factor_i = 6'd8;     // 配合量化格式的 scale 移位因子
        base_token_idx_i = 8'd0;     
        channel_tile_idx_i = 5'd23;  // 設定為最後一組 Channel Tile 以順利觸發 RMS 統計輸出
        token_valid_mask_i = 16'hFFFF; 
        
        // 將殘差 BRAM 展開至 2048-bit 寬度埠口
        ...
        $display("[System-TB] Systolic Array is IDLE. Asserting 'en' for Attn Head Score V...");
        en = 1'b1;
        @(posedge clk);
        en = 1'b0;
        ...
        fork
            ...
            begin
                ...
                // 【關鍵修正】多等 1 個時脈沿，確保最後一筆 Token 15 的非阻塞賦值成功寫入暫存器
                @(posedge clk); 
    ...
    // =================================================================
    // 6. 自動化資料比對與驗證任務 (Verification Task)
    // =================================================================
    ...
    task automatic check_data_result();
        ...
        begin
            for (int i = 0; i < 256; i++) begin
                ...
                if (current_hardware_out !== golden_attn_v_mem[i]) begin
                    if (error_count < 12) begin
                        $error("Data Mismatch at Pixel %0d: Expected [0x%02X], Got [0x%02X]", 
                                i, golden_attn_v_mem[i], current_hardware_out);
                    end
                    error_count++;
                end
            end
            ...
            if (error_count == 0) begin
                $display(">> [PASSED] SUCCESS: Attention Output Map (X_mid) matches Golden Model perfectly!");
            end else begin
    ...
    task automatic check_stat_result();
        ...
        begin
            ...
            if (error_count == 0) begin
                $display(">> [PASSED] SUCCESS: Attention RMSNorm Sum-of-Squares matches Golden Model perfectly!");
            end else begin
